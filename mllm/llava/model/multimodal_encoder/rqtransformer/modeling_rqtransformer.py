...
class RQTransformer(PreTrainedModel):
    ...
    _no_split_modules = ["head_transformer", "pos_emb_d", "classifiers"]
    
    def __init__(self, config: RQTransformerConfig):
        super().__init__(config)
        print("Initializing RQTransformer with config", "xxx"*20, config.to_dict())
        self.code_depth = config.code_depths
        ...
        self.pos_emb_d = nn.Parameter(torch.zeros(1, config.code_depths[0], config.embed_dim))
        ...
        self.classifiers = nn.ModuleList([
            ...
            for _ in range(config.code_depths[-1])
        ])
    ...
    def forward(self, embed_from_body, code, model_aux=None):
        ...
        logits = []
        for i in range(D):
            out_i = head_outputs[:, :, i, :]   # [B, seq_len, embed_dim]
            logits.append(self.classifiers[i](out_i))

        head_outputs = torch.stack(logits, dim=2)

        return head_outputs
    
    def get_the_codebook_idx(self, depth_id):
        return depth_id
    ...
    def generate(self, embed_from_body, model_aux=None, cfg=3.0):
        ...
        logits = self.classifiers[0](head_outputs)
        ...
        for i in range(self.pos_emb_d.shape[1]-1):
            ...
            book_i = self.get_the_codebook_idx(i + 1)
            logits = self.classifiers[book_i](head_outputs)
            logits = logits[B//2:, :] + cfg * (logits[:B//2, :] - logits[B//2:, :])
