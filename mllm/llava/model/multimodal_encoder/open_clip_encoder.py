...
class SigLipVisionTower(nn.Module):
    def __init__(self, ckpt_path=None):
        ...
        ckpt_path = ckpt_path if ckpt_path is not None else "google/siglip-so400m-patch14-384"
        
        self.load_model(ckpt_path)
        self.embed_dim = 1024 if "256" in ckpt_path else 1152
        self.n_embed = 256
    ...
    def load_model(self, ckpt_path=None, device_map=None):
        if self.is_loaded:
            print('openclip model is already loaded')
            return
        print(f"Load vision tower from {ckpt_path}")
